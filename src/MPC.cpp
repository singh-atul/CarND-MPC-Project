...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //Cost based on refrecnce state x i.e for CTE, psi_err and velocity
     fg[0] += 2 * CppAD::pow(vars[cte_start + t], 2);
     fg[0] += 500 * CppAD::pow(vars[epsi_start + t], 2);
     fg[0] += 500 * CppAD::pow(vars[v_start + t] - ref_vel, 2);
   ...
   //
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      //Storing the third degree polynomial
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  res_list.push_back(solution.x[delta_start]);
  res_list.push_back(solution.x[a_start]);

  for(unsigned int i=0;i<N;i++)
  {
    res_list.push_back(solution.x[x_start+i]);
    res_list.push_back(solution.x[y_start+i]);
  }
